...
History: PJN / 21-03-2003 1. Significant rework following comprehensive testing with the Tray icon class.
         ...
         PJN / 01-11-2004 1. Fixed a number of warnings when the code is compiled using the Detect 64 bit 
                          portability issues in Visual Studio .Net. Please note that the code as it 
...
#pragma warning( disable : 4244)	// /Wp64 errors which can occur because of bugs in 32 bit MFC, specifically call to SetWindowLongPtr below
...
BOOL CHookWnd::Hook(HWND hWnd)
{
  ...
  //Install the thunk
	...
	WNDPROC pfnWndProc = (WNDPROC) SetWindowLongPtr(hWnd, GWL_WNDPROC, (LONG_PTR)pProc);
	if (pfnWndProc == NULL)
		return FALSE;
  if (pFirstHook)
  {
  ...
  if (m_hWnd)
  {
    ...
    if (pFirstHook == this)
    {
      ...
      if (pFirstHook->m_pNextHook)
      {
        ...
        SetWindowLongPtr(m_hWnd, GWL_WNDPROC, (LONG_PTR)pProc);
      ...
      {
		    SetWindowLongPtr(m_hWnd, GWL_WNDPROC, (LONG_PTR)m_pfnSuperWindowProc);
