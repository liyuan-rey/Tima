...
public:
	...
	void SetBaseKeyName(HKEY hKeyParent, LPCTSTR KeyName)
	{
		...
		_tcsncpy(m_sKey, KeyName, MAX_PATH);
	}
...
public:
	...
	inline bool SaveLoadItem(LPCTSTR szName, TCHAR* Variable, ULONG& Size, bool bSave)
	{
		DWORD dwType = REG_SZ;
		if(bSave)
			return ERROR_SUCCESS == ::RegSetValueEx(m_hKey, szName, NULL, dwType, (LPBYTE)Variable, Size);
		else
		{
			DWORD ErrorCode = ::RegQueryValueEx(m_hKey, szName, NULL, &dwType, (LPBYTE)Variable, &Size);
