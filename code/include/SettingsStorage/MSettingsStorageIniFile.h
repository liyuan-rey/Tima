...
public:
	...
	CSettingsStorage *CreateSubStorage(LPCTSTR SubName)
	{
		...
		_stprintf(SubSectionName, _T("%s.%s"), m_SectionName, SubName);
	...
	{
		if (IniName) _tcsncpy(m_IniFileName, IniName, MAX_PATH);
		if (SectionName) _tcsncpy(m_SectionName, SectionName, MAX_PATH);
	}
