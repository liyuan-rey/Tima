...
History: PJN / 16-11-1998 1. m_nOriginateTime was getting set incorrectly in the SNTP response
         ...
         PJN / 17-03-2005 1. Updated to work with the latest release of CWSocket. Thanks to 
                          Alex Dron for reporting this issue.
                          2. Addition of a preprocessor macro "PJNSNTP_EXT_CLASS" which allows
                          the code to be easily added to an extension DLL.
                          3. General tidy up of the code in CSNTPClient::GetServerTime
                          4. Replaced the macro _WIN32_WCE with PJNSNTP_NO_PRIVILEDGE_CODE. This 
...
BOOL CSNTPClient::GetServerTime(LPCTSTR pszHostName, NtpServerResponse& response, int nPort)
{
  ...
  {
    if (m_bProxy)
      socket.Create(FALSE); //TCP if going thro a proxy
    else
      socket.Create(TRUE); //UDP otherwise
  }
  catch(CWSocketException* pEx)
  {
    TRACE(_T("CSNTPClient::GetServerTime, Failed to create client socket, Error:%d\n"), pEx->m_nError);
  ...
  try
  {
    if (m_sLocalBoundAddress.GetLength())
      socket.Bind(0, m_sLocalBoundAddress);

    if (m_bProxy)
    {
      if (m_sProxyUserName.GetLength())
        socket.ConnectViaSocks5(pszHostName, nPort, m_sProxyServer, m_nProxyPort, m_sProxyUserName, m_sProxyPassword, m_dwTimeout, TRUE);
      else
        socket.ConnectViaSocks5(pszHostName, nPort, m_sProxyServer, m_nProxyPort, NULL, NULL, m_dwTimeout, TRUE);
    }
    else    
    {
      socket.Connect(pszHostName, nPort);
    }
  }
  catch(CWSocketException* pEx)
  {
    TRACE(_T("CSNTPClient::GetServerTime, Could not connect to the SNTP server %s on port %d, Error:%d\n"), pszHostName, nPort, pEx->m_nError);
  ...
  try
  {
    socket.Send(&nbi, nSendSize);
  }
  catch(CWSocketException* pEx)
  {
    TRACE(_T("CSNTPClient::GetServerTime, Failed in call to send NTP request to the SNTP server, Error:%d\n"), pEx->m_nError);
  ...
  try
  {
    bReadable = socket.IsReadible(m_dwTimeout);
  }
  catch(CWSocketException* pEx)
  {
    pEx->Delete();
  ...
  if (!bReadable)
  {
    TRACE(_T("CSNTPClient::GetServerTime, Unable to wait for NTP reply from the SNTP server, Error:%d\n"), WSAETIMEDOUT);
  ...
  try
  {
    socket.Receive(&nfp, nReceiveSize);
  }
  catch(CWSocketException* pEx)
  {
    TRACE(_T("CSNTPClient::GetServerTime, Unable to read reply from the SNTP server, Error returns %d\n"), pEx->m_nError);
  ...
  response.m_RoundTripDelay = (response.m_DestinationTime - response.m_OriginateTime) - (response.m_TransmitTime - response.m_ReceiveTime);
...
#ifndef PJNSNTP_NO_PRIVILEDGE_CODE
BOOL CSNTPClient::EnableSetTimePriviledge()
{
  ...
  if (!bOpenToken) 
  {
    ...
    TRACE(_T("CSNTPClient::EnableSetTimePriviledge, Failed to get Adjust priviledge token, Error:%d\n"), GetLastError());
    return FALSE;
  }
  ...
  {
    TRACE(_T("CSNTPClient::EnableSetTimePriviledge, Failed in callup to lookup priviledge, Error:%d\n"), GetLastError());
    return FALSE;
  }
  ...
  BOOL bSuccess = AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0);
	if (!bSuccess)
	  TRACE(_T("CSNTPClient::EnableSetTimePriviledge, Failed to adjust SetTime priviledge, Error:%d\n"), GetLastError());
...
void CSNTPClient::RevertSetTimePriviledge()
{
  if (m_bTakenPriviledge) 
  {
    ...
    if (!AdjustTokenPrivileges(m_hToken, FALSE, &m_TokenPriv, 0, NULL, 0))
		  TRACE(_T("CSNTPClient::RevertSetTimePriviledge, Failed to reset SetTime priviledge, Error:%d\n"), GetLastError());
    CloseHandle(m_hToken);
  }
}
#endif //PJNSNTP_NO_PRIVILEDGE_CODE
...
BOOL CSNTPClient::SetClientTime(const CNtpTime& NewTime)
{
  ...
  if (EnableSetTimePriviledge())
  {
    ...
    if (!bSuccess)
      TRACE(_T("CSNTPClient::SetClientTime, Failed in call to set the system time, Error:%d\n"), GetLastError());
#ifndef PJNSNTP_NO_PRIVILEDGE_CODE
  }
